...
export default function ArticleOldFormModal(props: Props) {
  ...
  const [pageType, setPageType] = useState<PageType>(initial?.pageType ?? 'HOME_TEASER');
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'create') {
      props.onSave({ title, content, pageType, language, imageIds, tagIds });
    } else {
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <label style={{ ...labelStyle, flex: 1 }}>
              ...
              >
                {PAGE_VALUES.map((v) => (
                  <option key={v} value={v}>{t(`article.pageType.${v}`)}</option>
                ))}
              </select>
            </label>
